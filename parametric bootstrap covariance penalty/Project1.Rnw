...
Therefore,
\[
\widehat\sigma^2 = \Sexpr{sigmahat}.
\]
...
par(mar = c(3, 3, 3, 3)) 
plot(1:k1, regressrisk,pch=16,
     xlab = "Number of bins", ylab = "Estimated error")
@
The optimal number of bins is $\Sexpr{which.min(regressrisk)}$.
\begin{figure}
\begin{center}
...
@
We have $\widehat{\sigma}=\Sexpr{sigmahat}$. For every pair $(x_i,y_i)$, we generate a large number $B$ of simulated observations according to the approximated normal distribution $N(g(x_i),\widehat\sigma)$ and calculate the covariance penalty according to equation (2). To be more precise, for $b=1,...,B$, we generate $y^{*b}$ according to 
\[
...
@
The optimal number of knots is $\Sexpr{which.min(Errhat)}$.
\begin{figure}
\begin{center}
...
@
We get the optimal bandwidth is $\Sexpr{which.min(Errhat)*0.1}$. One thing to notice is that the fitting result of kernel regression is very bad here and the estimated error keeps decreasing as the bandwidth increasing. Fitting this set of data using kernel regression is questionable and the optimal kernel regressio model derived in this setting is not trustworthy. 
\begin{figure}
\begin{center}
\caption{Estimated errors of the linear spline models with different number of knots}
...
@
We get the optimal bandwidth is $\Sexpr{which.min(Errhat)*0.05}$.
\begin{figure}
\begin{center}
